...
> Same spec discipline as `prd` (see its appendix): hard cases first, mandatory Out-of-scope, mechanized invariants. Interaction specs must cover all states — loading / error / empty / timeout; the AI-grading-failure branch (keep the submission, silent retry, no wrongful quota deduction) is the canonical example.
