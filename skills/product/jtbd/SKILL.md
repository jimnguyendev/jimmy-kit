...
## Applied context (edtech example — not part of the original)
> Blocks: jumping to solutions before the job is clear ("a hammer looking for nails"). Signature practice: score a screen against its jobs — list the screen's 2–3 user jobs, grade every UI block against each job, flag blocks that serve none.
