...
- Published: https://github.com/jimnguyendev/jimmy-kit (public, MIT), tag v0.1.0. Install channels: `npx skills add jimnguyendev/jimmy-kit` (verified, 48 found), Claude Code plugin via `.claude-plugin/` (validated --strict, loads via --plugin-dir), clone+symlink scripts. Bumping a release: update version in `.claude-plugin/plugin.json` + `marketplace.json` `ref`, then tag.
